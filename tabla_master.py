...
def insertIdToFunc(id, type_data, id_funcion, direccion, param=None, dimensionada=0, matriz=0):
    if len(simbolos[id_funcion].value) >= 1 and not itFoundGlobalVar(id) and not itFoundLocal(id, id_funcion):
        simbolos[id_funcion].value[id] = tabla.tabla_local(type_data, None, direccion, param, dimensionada, matriz)
...
def show():
    for keys in simbolos:
        ...
        for id in simbolos[keys].value:
            print("id:", id)
            if simbolos[keys].value[id].dimensionada == 0 and simbolos[keys].value[id].matriz == 0:
            ...
            if simbolos[keys].value[id].dimensionada > 0 and simbolos[keys].value[id].matriz > 0:
                print("valor:", simbolos[keys].value[id].value, " type data:", simbolos[keys].value[id].type_data, " MEMORIA:", simbolos[keys].value[id].direccion, "DIME M:", simbolos[keys].value[id].dimensionada, "DIME N:", simbolos[keys].value[id].matriz)
            if simbolos[keys].value[id].dimensionada > 0 and simbolos[keys].value[id].matriz == 0:
